...
__global__ void modulus_kernel2(int length_lines, cufftComplex *PostFFTData, float* z) 
{
    //delete compute
    unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
    if(x<length_lines)
...
__global__ void modulus_kernel3(int length_lines, cufftComplex *PostFFTData, float* z) 
{
    //changeAPI
    unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
    if(x<length_lines)
        z[x] = sqrtf(PostFFTData[x].x *PostFFTData[x].x + PostFFTData[x].y *PostFFTData[x].y);
}
...
int main() {
    ...
    int FFTlength_lines = FFTlength*lines;
    // Launch kernel
    for(int i=0;i<10;i++){
        modulus_kernel<<<dimGrid, dimBlock>>>(FFTlength, lines, d_PostFFTData, d_z);
    }
    for(int i=0;i<10;i++){
        modulus_kernel1<<<dimGrid, dimBlock>>>(FFTlength, lines, d_PostFFTData, d_z);
    }
    for(int i=0;i<10;i++){
        modulus_kernel2<<<dimGrid, dimBlock>>>(FFTlength_lines, d_PostFFTData, d_z);
    }
    for(int i=0;i<10;i++){
        modulus_kernel3<<<dimGrid, dimBlock>>>(FFTlength_lines, d_PostFFTData, d_z);
    }
