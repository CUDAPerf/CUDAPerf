...
__global__ 
void test_double(short* in, float* out, int a, int b, int c, int d, int e)
{
...
int test()
{
    ...
    cudaEventRecord(start); 
    test_double<<<blocks, threads_per_block>>>(d_in, d_out, a, b, c, d, e);
    cudaEventRecord(stop); 
    cudaEventSynchronize(stop); 
    float elapsed_time_double;
    cudaEventElapsedTime(&elapsed_time_double, start, stop); 
    std::cout << "test_double kernel execution time: " << elapsed_time_double << " ms" << std::endl;
