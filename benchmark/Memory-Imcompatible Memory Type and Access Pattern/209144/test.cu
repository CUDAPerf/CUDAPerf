...
bool checkResults(float *result1, float *result2, int size) {
    for (int i = 0; i < size; i++) {
...
int main() {
    ...
    bool resultsEqual = checkResults(h_out1, h_out2, ngroups);
    
    std::cout << "Results are " << (resultsEqual ? "equal" : "not equal") << std::endl;
