...
int test() {
    ...
    float *d_in, *d_weights, *d_bias, *d_out_original, *d_out_optimized;
    ...
    bool same_result = compare_arrays(h_out_original, h_out_optimized, output_size);
    if (same_result) {
        std::cout << "The outputs of both kernels are the same." << std::endl;
    } else {
        std::cout << "The outputs of the kernels are different." << std::endl;
    }
