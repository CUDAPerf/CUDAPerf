...
// define col_access
__global__
void col_access_gmem(int n, float *L, float *r, int lda)
{
    int tx = threadIdx.x;
    int start_L = blockIdx.x * (lda * n);
    int start_r = blockIdx.x * lda;
...
float launch(int n, float *d_L, float *d_r, int lda, int batch_count, std::string mode) 
{
    ...
    if (mode == "col_access") {
        ...
        col_access_gmem<<<batch_count, 256>>>(n, d_L, d_r, lda);
...
int main() {
    ...
    for(int i=0;i<10;i++)
    {
        float time_row = launch(n, d_L, d_r, lda, batch_count, "row_access");
        float time_col = launch(n, d_L, d_r, lda, batch_count, "col_access");
        std::cout << "Row-wise access time: " << time_row << " ms" << std::endl;
        std::cout << "Column-wise access time: " << time_col << " ms" << std::endl;
    }
