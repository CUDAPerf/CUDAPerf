...
__global__ void dropr2(float *A, curandState *globalstate, uint64_t N, float R) {
...
void measureKernelTime(float *A, curandState *states, uint64_t N, float R) {
    ...
    dropr2<<<blocks, threadsPerBlock>>>(d_A, d_states, N, R);
    cudaEventRecord(stop);
    
    cudaEventSynchronize(stop);
    float time2;
    cudaEventElapsedTime(&time2, start, stop);
    std::cout << "Time for dropr2: " << time2 << " ms" << std::endl;
