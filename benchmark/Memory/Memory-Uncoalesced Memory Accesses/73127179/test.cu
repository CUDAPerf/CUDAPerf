...
__global__ void gpu_matrix_fma_reduction_improved(double *matrix, int rows, int cols, double u, double p) {
...
int main() {
    ...
    double *h_matrix_original = (double*)malloc(matrix_size);

    
    for (int i = 0; i < rows * cols; ++i) {
        h_matrix[i] = static_cast<double>(rand()) / RAND_MAX * 10000;
        h_matrix_original[i] = h_matrix[i];
    }
    ...
    checkCudaError(cudaMemcpy(d_matrix, h_matrix_original, matrix_size, cudaMemcpyHostToDevice), "cudaMemcpy failed");

    
    start = std::chrono::high_resolution_clock::now();
    for(int i = 0; i < 10; i++)
        gpu_matrix_fma_reduction_improved<<<(rows * cols + 255) / 256, 256>>>(d_matrix, rows, cols, u, p);
    checkCudaError(cudaDeviceSynchronize(), "Kernel launch failed for improved");
    end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> elapsed_improved = end - start;
    ...
    std::cout << "Improved kernel time: " << elapsed_improved.count() << " seconds" << std::endl;

    
    cudaFree(d_matrix);
    free(h_matrix);
    free(h_matrix_original);
