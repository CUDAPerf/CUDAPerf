...
__device__ unsigned int udiv_lt_100000_use (unsigned int x, unsigned int y)
{
...
__global__ void divtest1 (unsigned int *result, int len)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        for (unsigned int divisor = 1; divisor < LEN; divisor++) {
            ...
            unsigned int res = udiv_lt_100000_use (dividend, divisor);
            flag += res != ref;
        }
        result [i] = flag;
    }
...
int test (void)
{
    ...
    divtest1<<<dimGrid,dimBlock>>>(d_r, LEN);
